		...
		while (true)
		{
			...
			else if (c == 13) // Enter
			{
				...
				if (++columnIter != (*rowIter).end())
				{
					columnIter = temp;
					columnIter++;
					for (auto it = columnIter; it != (*rowIter).end(); it++)
					{
						if ((*it) == ' ' && ++it == (*rowIter).end())
						{
							continue;
						}
						data.push_back(*it);
					...
					list<char>::iterator endIter = next(columnIter, size);
					(*rowIter).erase(columnIter, endIter);
					...
					while (!data.empty())
					{
						auto temp = columnIter;
						columnIter++;
						(*rowIter).insert(columnIter, data.front());
						columnIter = ++temp;
						currentColumn++;
